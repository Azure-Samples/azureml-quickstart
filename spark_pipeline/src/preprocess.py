...
def main():
    ...
    print(f"**** Unparsed Args: {unparsed_args}")
    print("parsed_args.train_file_path:", parsed_args.train_file_path)

    if os.path.isdir(parsed_args.train_file_path):
        print("os.listdir()", os.listdir(parsed_args.train_file_path))
        json_file = [f for f in os.listdir(parsed_args.train_file_path) if ".json" in f][0]
        print("json_file:", json_file)
        parsed_args.train_file_path = f"{parsed_args.train_file_path}/{json_file}"
        print("Now parsed_args.train_file_path:", parsed_args.train_file_path)

        print("is it dir?", os.path.isdir(parsed_args.train_file_path))


    elif os.path.isdir(parsed_args.train_file_path):
        print("Reading the file....")
        print(open(parsed_args.train_file_path).read()[:100])
    else:
        print("file path is neither a file or dir!")
